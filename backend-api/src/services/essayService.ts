...
export const submitEssay = async (input: SubmitEssayInput): Promise<IEssay> => {
  ...
  // ── Compute attemptNumber ──────────────────────────────────────────
  if (assignmentId) {
    // Assignment-based: count previous attempts for this assignment
    ...
    // Free-write: count previous free-writes for this student
    const previousFreeWrites = await Essay.countDocuments({
      studentId: new mongoose.Types.ObjectId(studentId),
  ...
  if (assignmentId) {
    ...
    if (!centerId) {
      throw new AppError(
        "Bài tập do trung tâm tạo — bạn cần tài khoản trung tâm để nộp bài",
        403,
      );
    }
    ...
    // Max attempts check (already computed attemptNumber above)
...
export const saveGradingResult = async (
  ...
  const { overallScore, scoreBreakdown, feedback, grammarErrors, suggestions } =
    result;
